...
const home = () => {
  ...
  unitToggle.addEventListener("click", () => {
    if (localStorage.getItem("weather") !== null) {
      toggleMetric(getWeather());
    } else {
      return;
    }
  });

  const toggleMetric = (weather) => {
    const { units, temp, tempMax, tempMin, feelsLike } = weather;
    const degree = unitToggle.checked ? "°F": "°C";

    const toFahr = celDeg => { return Math.round(celDeg * 1.8 + 32) }
    const toCels = fahrDeg => { return Math.round((fahrDeg - 32)/1.8)}
